...
// base.js => Elements
let newGame, create, createCount, player1, player2,
	activePlayer, player1hp, player2hp, player1name, player2name,
	battleReady, rogue, berserker, engineer, scribe, other, statParameters = [],
	classes, newGameAlert, radioChoice, nameChoice, navbar, navTop, newGameBar;
console.log('log');
newGameAlert = false;
...
const init = () => {
	...
	createCount = 0;
	battleReady = true;
	activePlayer = 0; //  implement turn counter
...
const nextPlayer = () => {
	...
	if(activePlayer === 1){
		...
		elements.buff1.disabled = false;
		elements.hpCheck1.disabled = false;
		if(player2.buff)elements.special2.disabled = true;
		if(player1.buff)elements.special1.disabled = false;
		...
		elements.buff2.disabled = false;
		elements.hpCheck2.disabled = false;
		if(player2.buff)elements.special2.disabled = false;
		if(player1.buff)elements.special1.disabled = true;
...
[elements.newGame, elements.newGameBar].forEach(event => event.addEventListener('click', () => {
	if (player1.buff){
		player1.buffEffect(false);
		elements.special1.disabled = true;

	}
	if (player2.buff){
		player2.buffEffect(false);
		elements.special2.disabled = true;

	}
	console.log(player2.str);
	init();
	nextPlayer();
...
[elements.special1, elements.special2].forEach(event => event.addEventListener('click', () => {
	if (battleReady) {
		...
		eCond === 'p1-special' ? elements.updatehp2.textContent = player2hp : elements.updatehp1.textContent = player1hp
		elements.battleMessage.textContent = `
...
elements.buff2.addEventListener('click', () => {
	...
	if (player2.buff) {
		elements.battleMessage.textContent = `${player2name} is ${player2.text}`;
